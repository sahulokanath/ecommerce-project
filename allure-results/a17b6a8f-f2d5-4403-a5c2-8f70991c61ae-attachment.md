...
```
TypeError: addToWishList.successMessage is not a function
```
...
```ts
  ...
  44 |     
  45 |     // Navigate to wish list
  46 |     await addToWishList.wishList(); 
  47 | 
  48 |     // Validate that the product is added to the wish list
  49 |     const productName = await addToWishList.productName();
  50 |     expect(productName).toBe("iPhone");
  51 | 
  52 |     // Validate success message
> 53 |     const successMessage = await addToWishList.successMessage();
     |                                                ^ TypeError: addToWishList.successMessage is not a function
  54 |     expect(successMessage).toContain("Success: You have added iPhone to your wish list!");
  55 | });
```