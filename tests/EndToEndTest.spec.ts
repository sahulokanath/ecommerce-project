...
test('execute end-to-end test flow @end-to-end @master', async ({ page }) => {
...
async function performRegistration(page: Page): Promise<string> {
    ...
    let email: string = RandomDataGenerator.generateRandomEmail();
...
async function performLogout(page: Page) {
    ...
    const logoutPage: Logout = await myAccountPage.clickLogout();

    // Ensure the "Continue" button is visible
    expect(await logoutPage.continueButtonIsDisplayed()).toBe(true);
...
async function addProductToCart(page: Page) {
    ...
    await homePage.enterSearchTerm(productName);
