...
```ts
  ...
  12 | const loginData=DataProvider.getTestDataFromJson(jsonPath);
  13 | 
  14 | 
  15 | for (const data of loginData) {
  16 |     test(`Login Test with username: ${data.username} and password: ${data.password} @datadriven` , async ({ page }) => {
  17 | 
...
> 29 |         if(data.expected.toLowerCase() === "success") {
  ...
  31 |             expect(await myAccountPage.isMyAccountPageDisplayed()).toBeTruthy();
  32 |         } else {
  33 |             const errorMsg = await loginPage.getErrorMsg();
  34 |             expect(errorMsg).toContain("Warning: No match for E-Mail Address and/or Password.");
  35 |         }
  36 | 
  37 | 
  38 |         
