...
- Name: LoginDataDrivern.spec.ts >> Login Test with username: testuser@example.com and password: Password123 @datadriven
- Location: tests\LoginDataDrivern.spec.ts:13:9
        ...
        - generic [ref=e87]:
          ...
          - generic [ref=e91]:
            - generic [ref=e92]:
              ...
              - textbox "E-Mail Address" [ref=e94]: testuser@example.com
            - generic [ref=e95]:
              - generic [ref=e96]: Password
              - textbox "Password" [ref=e97]: Password123
...
```ts
  ...
  10 | const loginData=DataProvider.getUserData(jsonPath);
  11 | 
  12 | for (const data of loginData) {
  13 |     test(`Login Test with username: ${data.username} and password: ${data.password} @datadriven` , async ({ page }) => {
  14 | 
  15 |         const testConfig = new TestConfig();
  ...
  25 | 
> 26 |         if(data.expectedResult.toLowerCase() === "success") {
     |                                ^ TypeError: Cannot read properties of undefined (reading 'toLowerCase')
  27 |             const myAccountPage = new MyAccountPage(page);
  28 |             expect(await myAccountPage.isMyAccountPageDisplayed()).toBeTruthy();
  29 |         } else {
  30 |             const errorMsg = await loginPage.getErrorMsg();
  31 |             expect(errorMsg).toContain("Warning: No match for E-Mail Address and/or Password.");
  32 |         }
  33 | 
  34 | 
  35 |         
