...
```ts
  ...
  18 | 
  19 | test('Account  Registion', async ({ page }) => {
  20 | 
  21 |     const testConfig = new TestConfig();
  22 | 
  23 |     await page.goto(testConfig.appUrl); // navigate to the application URL
  24 | 
  25 |     const homePage = new HomePage(page);
> 26 |     await homePage.clickMyAccount(); // click on my account link
     |                    ^ TypeError: homePage.clickMyAccount is not a function
  27 |     await homePage.clickRegister();// click on register link
  28 | 
  29 |     const registrationPage = new RegistrationPage(page);
  30 | 
  31 |     registrationPage.enterFirstName(RandomDataGenerator.generateRandomFirstName());
  32 |     registrationPage.enterLastName(RandomDataGenerator.generateRandomLastName());
  33 |     registrationPage.enterEmail(RandomDataGenerator.generateRandomEmail());
  34 |     registrationPage.enterTelephone(RandomDataGenerator.generateRandomPhoneNumber());
  35 |     const password = RandomDataGenerator.generateFullName();
  36 | 
  37 |     // Two password should be same for registration
  38 |     registrationPage.enterPassword(password);
  39 |     registrationPage.enterConfirmPassword(password);
  40 | 
  41 |     await registrationPage.clickPrivacyPolicy();
  42 |     await registrationPage.clickContinue();
  43 | 
  44 |     const conformation= registrationPage.getConformationMsg();
  45 | 
  46 |      expect(conformation).toContain("Your Account Has Been Created!");
  47 | 
  48 |      await page.waitForTimeout(3000); // wait for 3 seconds to see the result
  49 | });
```