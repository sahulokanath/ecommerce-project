...
```
TypeError: homePage.enterSearchTerm is not a function
```
...
```ts
  ...
  8  | let searchResultsPage: SearchResultsPage;
  9  | 
  ...
  17 | });
  18 | 
  19 | test('Search Product @master @regression', async () => {
  20 | 
  21 |     const searchTerm = testConfig.searchTerm;
  22 | 
> 23 |     await homePage.enterSearchTerm(searchTerm);
     |                    ^ TypeError: homePage.enterSearchTerm is not a function
  24 |     await homePage.clickSearchButton();
  25 | 
  26 |     const isProductDisplayed =
  27 |         await searchResultsPage.isProductDisplayed(searchTerm);
  28 | 
  29 |     expect(isProductDisplayed).toBeTruthy();
  30 | });
```